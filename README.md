...
ExamSplit AI 支持三种本地离线切题方式，即使在**无网络、无外部商业 API** 的环境下也能顺畅运行：

#### 模式一：极速规则模式（0 门槛 / 无需下载任何模型 / 最推荐日常使用）
- **核心优势**：完全不需要下载任何几 GB 的大模型，不消耗 GPU 显存，纯本地 CPU 毫秒级秒切！
- **底层原理**：内置工业级试卷版面拓扑分析器（`ReadingOrderDetector`）与自适应物理题号图扫描（`LocalMarkerDetector`），对 90% 以上标准排版试卷（如考研数学一二三、高考数学、物理等）切分精度媲美云端大模型！
- **操作步骤**：
  1. 打开软件顶部【更多功能 ▾】->【🧪 本地切题实验仪表盘】；
  2. 将【运行模式】选择为：**极速规则模式 (仅本地规则，0秒VLM延迟)**；
  3. 点击【🚀 开始本地切题分析】，秒级完成全卷切题；
  4. 点击【✅ 应用实验结果到工作区】即可直接在主界面画布预览并无损导出。

#### 模式二：Ollama 一键托管运行（推荐本地拥有独显 GPU 的用户）
- **核心优势**：全自动 GPU 加速多模态视觉小模型，疑难复杂题目自动送审复核。
- **推荐模型**：`Qwen2.5-VL-3B-Instruct`（阿里通义千问 3B 轻量级视觉大模型，仅需约 3.5GB ~ 4.5GB 显存）。
- **操作步骤**：
  1. 前往 [Ollama 官网](https://ollama.com) 下载并安装 Ollama；
  2. 打开系统终端或 PowerShell，执行以下命令自动拉取并启动模型：
     ```bash
     ollama run qwen2.5vl:3b
     ```
  3. 启动 ExamSplit AI，打开【🧪 本地切题实验仪表盘】：
     - 服务端点保持默认：`http://127.0.0.1:11434/v1`
     - VLM 模型保持默认：`qwen2.5vl:3b`
     - 点击【测试连接】，提示“连接成功”即可；
  4. 点击【🚀 开始本地切题分析】，享受本地规则 + 本地小 VLM 复合高精切分！

#### 模式三：放入软件根目录 `models/` 文件夹并通过 llama-server 启动
- **核心优势**：纯单文件 GGUF 绿色便携，适合高级用户或离线局域网/服务器环境。
- **操作步骤**：
  1. 从 HuggingFace 或 ModelScope（魔搭社区）搜索并下载 GGUF 格式视觉模型：
     - 推荐下载：`Qwen2.5-VL-3B-Instruct-Q4_K_M.gguf`（约 3.0 GB）；
  2. 将下载好的 `.gguf` 文件直接放入本软件目录下的 **`models/`** 文件夹中（软件界面支持一键点击【📂 打开 models 目录】）；
  3. 启动 ExamSplit AI，打开本地实验仪表盘时，软件会自动扫描 `models/` 并在【检测 models/:】下拉列表中列出检测到的模型！
  4. 使用 `llama-server` 启动提供本地 OpenAI 兼容端点：
     ```bash
     # Windows 终端运行:
     llama-server.exe -m models\Qwen2.5-VL-3B-Instruct-Q4_K_M.gguf --port 11434 -ngl 99 -c 4096

     # Linux 终端运行:
     ./llama-server -m models/Qwen2.5-VL-3B-Instruct-Q4_K_M.gguf --port 11434 -ngl 99 -c 4096
     ```
  5. 软件中点击【测试连接】即可对接本地加速服务。
